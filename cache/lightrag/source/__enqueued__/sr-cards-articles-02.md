...
**Slug:** `sr-cards-articles-02`
**HubSpot module ID:** 210345737756
...
**Module path:** `sr-2026/custom-modules/SR Cards Articles 02.module`
...
- **cta** (group) — CTA
  - **ctas.default** (group — when cta.cta_style EQUAL custom) — Default
  - **ctas.hover** (group — when cta.cta_style EQUAL custom) — Hover
  - **cta.icon** (group — when cta.add_icon EQUAL true) — Icon
...
- **column.column_size** (group — repeater(default=1, max=4)) — Column Size
...
- `readmore.cta_size` (choice) — CTA Size [when readmore.cta_type NOT_EQUAL none] [required]
...
- `number_of_posts` (number) — Number of Posts
- `show_image` (boolean) — Show Image
