...
import browserExtractTypographyAndHero from "./lib/browser-extract-typography-hero.mjs";
...
// When "1", logs clipped OpenAI output text to the console (no file). Default off — logging is I/O + noise.
const OPENAI_LOG_RESPONSES = String(process.env.OPENAI_LOG_RESPONSES || "0") === "1";
...
const HERO_SETTLE_MS = Number(process.env.HERO_SETTLE_MS || 800);
const HERO_LOADSTATE_TIMEOUT_MS = Number(process.env.HERO_LOADSTATE_TIMEOUT_MS || 15000);
const PLAYWRIGHT_WAIT_UNTIL = new Set(["load", "domcontentloaded", "networkidle", "commit"]);
function envWaitUntil(name, fallback) {
  const v = String(process.env[name] || fallback || "").trim().toLowerCase();
  return PLAYWRIGHT_WAIT_UNTIL.has(v) ? v : fallback;
}
function optionalLoadStateEnv(name) {
  const v = String(process.env[name] || "").trim().toLowerCase();
  return PLAYWRIGHT_WAIT_UNTIL.has(v) ? v : null;
}
...
app.post("/api/typography", async (req, res) => {
  try {
    ...
    const result = await extractTypographyFromUrl(websiteUrl, {
      includeHeroScreenshot: Boolean(req.body?.include_hero_screenshot),
    });
...
app.get("/api/typography", async (req, res) => {
  try {
    ...
    const result = await extractTypographyFromUrl(websiteUrl, {
      includeHeroScreenshot: String(req.query?.include_hero_screenshot || "") === "1",
    });
    return res.json(result);
  } catch (error) {
...
function mergeTypographyHeroPrefer(pageTypo, heroTypo) {
  if (!heroTypo || typeof heroTypo !== "object") {
    return pageTypo;
  }
  const uniqueSlice = (arr, n) => [...new Set(arr.filter(Boolean))].slice(0, n);
  const heroBody = heroTypo.body;
  const pageBody = pageTypo.body;
  const useHeroBody =
    heroBody &&
    typeof heroBody === "object" &&
    String(heroBody.fontSize || heroBody.fontFamily || "").trim();
  return {
    ...pageTypo,
    ...heroTypo,
    body: useHeroBody ? heroBody : pageBody,
    fonts: {
      ...(pageTypo.fonts || {}),
      ...(heroTypo.fonts || {}),
      body: useHeroBody
        ? heroTypo.fonts?.body || pageTypo.fonts?.body
        : pageTypo.fonts?.body || heroTypo.fonts?.body,
    },
    headings: { ...(pageTypo.headings || {}), ...(heroTypo.headings || {}) },
    links: { ...(pageTypo.links || {}), ...(heroTypo.links || {}) },
    buttons: { ...(pageTypo.buttons || {}), ...(heroTypo.buttons || {}) },
    colors: {
      ...(pageTypo.colors || {}),
      ...(heroTypo.colors || {}),
      text: uniqueSlice(
        [...(pageTypo.colors?.text || []), ...(heroTypo.colors?.text || [])],
        10
      ),
      background: uniqueSlice(
        [...(pageTypo.colors?.background || []), ...(heroTypo.colors?.background || [])],
        10
      ),
    },
    google_fonts_urls: [
      ...new Set([...(pageTypo.google_fonts_urls || []), ...(heroTypo.google_fonts_urls || [])]),
    ],
    font_face_urls: [
      ...new Set([...(pageTypo.font_face_urls || []), ...(heroTypo.font_face_urls || [])]),
    ],
    font_stylesheet_urls: [
      ...new Set([
        ...(pageTypo.font_stylesheet_urls || []),
        ...(heroTypo.font_stylesheet_urls || []),
      ]),
    ],
  };
}

function absolutizeHeroAssetUrls(hero, baseUrlString) {
  ...
  if (hero.background?.recommendedImageUrl) {
    hero.background.recommendedImageUrl = abs(hero.background.recommendedImageUrl);
  }
  const list = hero.background?.imageUrls;
  if (Array.isArray(list) && list.length) {
    hero.background.imageUrls = list.map(abs);
  }
...
function absolutizeHeroCtas(hero, baseUrlString) {
  if (!Array.isArray(hero?.ctas) || !hero.ctas.length) {
    return;
  }
  let base;
  try {
  ...
  for (const c of hero.ctas) {
    if (!c || c.href == null) {
      continue;
    }
    const h = String(c.href).trim();
    if (!h || h === "#") {
      continue;
    }
    try {
      c.href = new URL(h, base).href;
    } catch {
      /* keep */
    }
  }
...
async function extractTypographyFromUrl(websiteUrl, options = {}) {
  const includeHeroScreenshot = Boolean(options.includeHeroScreenshot);
  let parsedUrl = null;
  try {
  ...
  try {
    const gotoOptions = { waitUntil: envWaitUntil("HERO_GOTO_WAIT_UNTIL", "domcontentloaded") };
    ...
    const afterState = optionalLoadStateEnv("HERO_AFTER_LOAD_STATE");
    if (afterState) {
      await page
        .waitForLoadState(afterState, { timeout: HERO_LOADSTATE_TIMEOUT_MS })
        .catch(() => {});
    }
    if (Number.isFinite(HERO_SETTLE_MS) && HERO_SETTLE_MS > 0) {
    ...
    const extracted = await page.evaluate(browserExtractTypographyAndHero);
    const typographyPage = extracted.typographyPage || {};
    const heroRaw = extracted.hero || { found: false };
    absolutizeHeroAssetUrls(heroRaw, finalPageUrl || parsedUrl.toString());
    absolutizeHeroCtas(heroRaw, finalPageUrl || parsedUrl.toString());

    let typographyData = typographyPage;
    if (heroRaw.found && heroRaw.typography) {
      typographyData = mergeTypographyHeroPrefer(typographyPage, heroRaw.typography);
    }

    let heroScreenshot = null;
    const hero = { ...heroRaw };
    if (includeHeroScreenshot && hero.found) {
      const loc = page.locator("[data-ditto-hero-cap]");
      try {
        if ((await loc.count()) > 0) {
          const buf = await loc.first().screenshot({ type: "jpeg", quality: 68 });
          heroScreenshot = {
            mime: "image/jpeg",
        ...
        console.warn("[API] hero screenshot failed:", e.message || e);
      }
    }
    await page.evaluate(() => {
      document.querySelectorAll("[data-ditto-hero-cap]").forEach((n) => n.removeAttribute("data-ditto-hero-cap"));
    });

    // Optional: write a local debug bundle (modules.json + extract → typography_matched_data.json).
    // This is cheap (usually under tens of ms) vs Playwright itself; it does **not** affect Ditto
    // purple/teal OpenAI routes. Uncomment if you want the JSON file on disk again.
    /*
    try {
      const modulesPath = path.join(__dirname, "modules.json");
      const raw = await readFile(modulesPath, "utf8");
      const parsed = JSON.parse(raw);
      const srModules = Array.isArray(parsed?.modules) ? parsed.modules : [];

      const matchedData = {
        timestamp: new Date().toISOString(),
        website_url: finalPageUrl || parsedUrl.toString(),
        page_title: pageTitle || "",
        typography_extracted: typographyData,
        typography_full_page: typographyPage,
        hero,
        sr_modules_analyzed: srModules.map((m) => ({
          name: m.name || "",
      ...
      const outPath = path.join(__dirname, "typography_matched_data.json");
      await writeFile(outPath, JSON.stringify(matchedData, null, 2), "utf8");
      console.log(`[API] Saved matching results to ${outPath}`);
    } catch (e) {
      console.error("[API] Failed to generate json file:", e.message);
    }
    */
    ...
    if (heroScreenshot) {
      out.hero_screenshot = heroScreenshot;
    }
    return out;
  } finally {
...
/** Same env knobs as Ditto `lib/hero-match-pipeline.js` — vision+layout `/v1/responses` only. */
function openAiResponsesLatencyExtras(model) {
  ...
  const out = {};
  const effRaw = String(process.env.OPENAI_RESPONSES_REASONING_EFFORT ?? "low").trim().toLowerCase();
  if (effRaw && effRaw !== "off") {
    const ok = new Set(["none", "low", "medium", "high", "xhigh"]);
    out.reasoning = { effort: ok.has(effRaw) ? effRaw : "low" };
  }
  const verbRaw = String(process.env.OPENAI_RESPONSES_TEXT_VERBOSITY ?? "low").trim().toLowerCase();
  if (verbRaw && verbRaw !== "off") {
    const okv = new Set(["low", "medium", "high"]);
    out.text = { verbosity: okv.has(verbRaw) ? verbRaw : "low" };
  }
  const tier = String(process.env.OPENAI_RESPONSES_SERVICE_TIER || "").trim().toLowerCase();
  ...
  try {
    response = await fetchWithRetry(OPENAI_API_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        ...openAiResponsesLatencyExtras(openaiModel || OPENAI_MODEL),
        input: [
          {
            role: "user",
            content: [
