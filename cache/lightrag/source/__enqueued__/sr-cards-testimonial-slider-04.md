...
**Slug:** `sr-cards-testimonial-slider-04`
**HubSpot module ID:** 210345392420
**Inferred category:** Cards (testimonials)
...
**Module path:** `sr-2026/custom-modules/SR Cards Testimonial Slider 04.module`
...
- **1** (group — repeater(default=6, max=∞)) — Slides
  - **video** (group — when col_media_type EQUAL video) — Video
    - **style_options** (group) — Style Options
    - **placeholder_fields** (group) — Placeholder text
  - **slide_heading** (group) — Heading
...
- `add_to_lightbox` (boolean) — Add to Lightbox [when col_media_type EQUAL image] — _Enable this toggle to add this image to a lightbox pop-up_
- `col_image` (image) — Image [when col_media_type EQUAL image]
- `image_corner_radius` (number) — Media Corner Radius [when col_media_type MATCHES_REGEX image|video AND add_to_lightbox NOT_EQUAL true]
- `image_link` (link) — Image Link [when col_media_type EQUAL image AND add_to_lightbox NOT_EQUAL true]
- `banner_image` (boolean) — Banner Image [when col_media_type EQUAL image AND add_to_lightbox NOT_EQUAL true]
- `banner_height` (number) — Banner Height [when banner_image EQUAL true AND col_media_type EQUAL image]
- `full_width_image` (boolean) — Full Width Image [when banner_image EQUAL false AND col_media_type EQUAL image] — _Stretch image to full width of the column_
...
- `video_autoplay` (boolean) — Autoplay [when video_type EQUAL html_video AND html_video NOT_EMPTY null]
...
- `slide_heading.heading` (text) — Heading
- `slide_heading.heading_size` (choice) — Size [required] — _HTML heading size_
- `slide_heading.heading_display_size` (choice) — Display Size [required] — _Size that heading is displayed at_
- `slide_heading.heading_color` (choice) — Color [required]
- `slide_heading.heading_color_custom` (color) — Color [when slide_heading.heading_color EQUAL custom]
- `slide_heading.heading_align` (choice) — Align [required]
- `slides.heading.class` (text) — CSS Class
- `slides.slide_content` (richtext) — Content
- `slides.slide_author_name` (text) — Author Name
- `slides.slide_author_title` (text) — Author Title
- `dots` (boolean) — Dots
- `arrows` (boolean) — Arrows
- `autoplay` (boolean) — Autoplay
- `autoplay_speed` (number) — Autoplay Speed [when autoplay EQUAL true] — _In seconds_
