...
function buildHeroCoverCssLines(hero) {
  ...
  const pos = String(bg.backgroundPosition || bg.backdrop?.backgroundPosition || "").trim();
  const size = String(bg.backgroundSize || bg.backdrop?.backgroundSize || "").trim();
  const rep = String(bg.backgroundRepeat || bg.backdrop?.backgroundRepeat || "").trim();
