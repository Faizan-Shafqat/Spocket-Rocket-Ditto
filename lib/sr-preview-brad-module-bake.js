...
const { buildHeroCoverCssLines } = require("./typography-hero-cover-css");
...
function buildBradModuleHeroCoverCss(hero) {
  ...
  const solid = String(bg.backgroundColor || bg.backdrop?.backgroundColor || "").trim();
  const pos = String(bg.backgroundPosition || bg.backdrop?.backgroundPosition || "center").trim();
  const size = String(bg.backgroundSize || bg.backdrop?.backgroundSize || "cover").trim();
  const rep = String(bg.backgroundRepeat || bg.backdrop?.backgroundRepeat || "no-repeat").trim();
